...
@swagger_auto_schema(
    method='post',
    request_body=get_conditional_schema('ServiceProvider_Admin'),  # Or dynamically based on request.user.user_type
    responses={
    ...
    operation_description="Admin can register users, send autogenerated credentials, and trigger email or OTP verification.",
    manual_parameters=[
        openapi.Parameter(
            'Authorization',
            openapi.IN_HEADER,
...
def users_creation(request):
    """
    Handles user registration by admin with autogenerated password.
    ...
    try:
        # Common logic for all admin users
        ...
        with transaction.atomic():
            ...
            # If ServiceProvider_Admin, handle additional logic
            if request.user.user_type == 'ServiceProviderAdmin':
                visa_applications_data = {}
                visa_applications_data['first_name'] = request.data.get('first_name')
                visa_applications_data['last_name'] = request.data.get('last_name')
                visa_applications_data['passport_number'] = request.data.get('passport_number')
                visa_applications_data['purpose'] = request.data.get('purpose')
                visa_applications_data['visa_type'] = request.data.get('visa_type')
                visa_applications_data['destination_country'] = request.data.get('destination_country')
                visa_applications_data['user'] = user_data.id
        ...
        logger.error(f"SES email sending error: {str(e)}")
        return Response(
...
class VisaApplicationsAPIView(APIView):
    ...
    @swagger_auto_schema(
        operation_description="Retrieve a list of all visa applications or details of a specific visa application by ID.",
        responses={
            200: "List of visa applications retrieved successfully or visa application details found.",
            404: "Visa application not found (for detail view)."
        }
    )
    def get(self, request, pk=None):
        if pk:
    ...
    @swagger_auto_schema(
        operation_description="Bulk create or add multiple services to a visa application.",
        request_body=openapi.Schema(
            type=openapi.TYPE_OBJECT,
            properties={
                'visaapplication_id': openapi.Schema(type=openapi.TYPE_INTEGER,
                                                     description='ID of the visa application'),
                'services': openapi.Schema(
                    type=openapi.TYPE_ARRAY,
                    items=openapi.Schema(
                        type=openapi.TYPE_OBJECT,
                        properties={
                            'service_type': openapi.Schema(type=openapi.TYPE_INTEGER,
                                                           description='Type of the service (service ID)'),
                    ...
                    description='List of services to add to the visa application'
                )
            },
            required=['visaapplication_id', 'services'],  # Both fields are required
            responses={
                201: "Services added successfully.",
                400: "Invalid data."
            }
        )
    )
    def post(self, request):
        # Extract the visa_application_id and the list of services from the request data
        visaapplication_id = request.data.get('visaapplication_id')
        services_data = request.data.get('services', [])

        # Get the VisaApplication instance by ID
        visa_application = get_object_or_404(VisaApplications, id=visaapplication_id)

        # Prepare the data for serialization and create new ServiceDetails
        for service in services_data:
            service[
                'visa_application'] = visa_application.id  # Assign the visa application to the service (ForeignKey assignment)

            # Serialize and save each service
