...
def upload_pan_card_file(instance, filename):
    if hasattr(instance, 'directors_ref'):
        return os.path.join('company_incorporation', 'pan_card', str(instance.directors_ref.service_request.id), filename)
    return os.path.join('company_incorporation', 'pan_card', str(instance.shareholders_ref.service_request.id), filename)

def upload_aadhaar_card_file(instance, filename):
    if hasattr(instance, 'directors_ref'):
        return os.path.join('company_incorporation', 'aadhaar_card', str(instance.directors_ref.service_request.id), filename)
    return os.path.join('company_incorporation', 'aadhaar_card', str(instance.shareholders_ref.service_request.id), filename)

def upload_passport_photo_file(instance, filename):
    return os.path.join('company_incorporation', 'passport_photo', str(instance.directors_ref.service_request.id), filename)

def upload_residential_address_proof_file(instance, filename):
    if hasattr(instance, 'directors_ref'):
        return os.path.join('company_incorporation', 'residential_address_proof', str(instance.directors_ref.service_request.id), filename)
    return os.path.join('company_incorporation', 'residential_address_proof', str(instance.shareholders_ref.service_request.id), filename)
...
def upload_specimen_signature_of_director(instance, filename):
    return os.path.join('company_incorporation', 'specimen_signature', str(instance.directors_ref.service_request.id), filename)

def upload_bank_statement_file(instance, filename):
    return os.path.join('company_incorporation', 'bank_statement', str(instance.shareholders_ref.service_request.id), filename)

def review_filing_certificate(instance, filename):
    return os.path.join('service_requests', 'company_incorporation', str(instance.service_request.id), 'review_filing_certificate', filename)
