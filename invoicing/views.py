...
def createDocument(request, id):
    try:
        ...
        context = {
            'company_name': invoice.invoicing_profile.business.email,
            ...
            'gst_registered': "Yes",
            'gstin': "gstin123456",
            'email': invoice.invoicing_profile.business.email,
            'mobile': invoice.invoicing_profile.business.mobile,
            'pan': invoice.invoicing_profile.pan_number,
            'bank_name': invoice.invoicing_profile.bank_name,
            'account_number': invoice.invoicing_profile.account_number,
            'ifsc_code': invoice.invoicing_profile.account_number,
            'invoice_format': invoice.invoicing_profile.ifsc_code,
            'swift_code': invoice.invoicing_profile.ifsc_code,
            'signature': signature_base64,
            # Invoice data fields (only for the selected invoice)
            'customer_name': invoice.customer,
            'terms': invoice.terms,
            'due_date': due_date_str,
            'financial_year': invoice.financial_year if invoice else '',
            'invoice_number': invoice.invoice_number if invoice else '',
            'invoice_date': formatStringDate(invoice.invoice_date) if invoice else '',
            'place_of_supply': invoice.place_of_supply if invoice else '',

            # Bill To address fields
            'bill_to_address': invoice.billing_address.get('address_line1', '') if invoice else '',
            'bill_to_state': invoice.billing_address.get('state', '') if invoice else '',
            'bill_to_country': invoice.billing_address.get('country', '') if invoice else '',
            'bill_to_pincode': invoice.billing_address.get('postal_code', '') if invoice else '',

            # Ship To address fields
            'ship_to_address': invoice.shipping_address.get('address_line1', '') if invoice else '',
            'ship_to_state': invoice.shipping_address.get('state', '') if invoice else '',
            'ship_to_country': invoice.shipping_address.get('country', '') if invoice else '',
            'ship_to_pincode': invoice.shipping_address.get('postal_code', '') if invoice else '',

            # Item Details
            'item_details': invoice.item_details,
            'total': invoice.total_amount if invoice else 0,
            'subtotal': f"{round(float(invoice.get('subtotal_amount', '0')), 2):.2f}",
            'cgst_amt': f"{round(float(invoice.get('cgst_amount', '0')), 2):.2f}",
            'sgst_amt': f"{round(float(invoice.get('sgst_amount', '0')), 2):.2f}",
            'total': total_str,
            'total_in_words': total_in_words,
            'cgst': round(float(invoice.get('cgst_amt', '0')), 2),
            'sgst': round(float(invoice.get('sgst_amount', '0')), 2),
            'igst_amt': invoice.get('igst_amt', 0) if invoice else 0,
            'payment_status': invoice.get('payment_status', '') if invoice else '',
            'terms_and_conditions': invoice.get('termsAndConditions', '') if invoice else '',
            'note': invoice.get('note', '') if invoice else '',
